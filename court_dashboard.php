<?php
/**
 * JailTrak - Court Case Dashboard
 * Exact copy of inmate dashboard but for court cases
 */
...
// Get statistics
$stats = [
    'total' => $db->query("SELECT COUNT(*) FROM court_cases")->fetchColumn(),
    'active' => $db->query("SELECT COUNT(*) FROM court_cases WHERE case_status LIKE '%Active%' OR case_status LIKE '%Pending%'")->fetchColumn(),
    'disposed' => $db->query("SELECT COUNT(*) FROM court_cases WHERE disposition IS NOT NULL AND disposition != ''")->fetchColumn(),
    'judges' => $db->query("SELECT COUNT(DISTINCT judge) FROM court_cases WHERE judge IS NOT NULL")->fetchColumn(),
...
<head>
    ...
    <title>Court Case Dashboard - JailTrak</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        ...
        <header>
            <h1>⚖️ Court Case Dashboard</h1>
            <p class="subtitle">Real-time court case tracking - Clayton County Superior Court</p>
        ...
        <div class="controls">
            ...
            <div class="filter-tabs">
                <a href="?filter=all&search=<?= urlencode($search) ?>" class="filter-btn <?= $filter === 'all' ? 'active' : '' ?>">All Cases</a>
                <a href="?filter=active&search=<?= urlencode($search) ?>" class="filter-btn <?= $filter === 'active' ? 'active' : '' ?>">Active</a>
                <a href="?filter=disposed&search=<?= urlencode($search) ?>" class="filter-btn <?= $filter === 'disposed' ? 'active' : '' ?>">Disposed</a>
            </div>
